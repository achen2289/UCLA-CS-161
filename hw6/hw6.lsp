...
; Each node gets k possible colors, 1 - k for 1st node, k+1 - 2k for 2nd node, ...
...
(defun at-most-one-color (n c k)
  (cond ((= c k) nil)
        (t (let ((negative_pairs_with_c (gen_neg_pairs n c (+ c 1) k)))
                (append negative_pairs_with_c (at-most-one-color n (+ c 1) k))
        ))
  )
)

(defun gen_neg_pairs (n start pair k)
  (cond ((> pair k) nil)
        (t (let ((neg_pair (list (- (node2var n start k)) (- (node2var n pair k)))))
                (cons neg_pair (gen_neg_pairs n start (+ pair 1) k))
        ))
  )
)
...
(defun generate-edge-clauses (e k)
  (gen-edge-constraints (car e) (cadr e) 1 k)
)

; Generates edge constraints which is nodes x and y cannot both be color c
(defun gen-edge-constraints (x y c k)
  (cond ((> c k) nil)
        (t (let ((edge_constraint (list (- (node2var x c k)) (- (node2var y c k)))))
                (cons edge_constraint (gen-edge-constraints x y (+ c 1) k))
        ))
  )
)
